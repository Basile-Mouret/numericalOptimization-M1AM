...
def level_2points_plot( f , x_tab, x_tab2, lb, ub ,nb_points, levels , title ):
    ...
    for k in range(x_tab.shape[0]):
	    plt.plot(x_tab[k,0],x_tab[k,1],'+g',markersize=10)

    for k in range(x_tab.shape[0]-1):
	    plt.plot([x_tab[k,0], x_tab[k+1,0]],[x_tab[k,1], x_tab[k+1,1]],'-g')

    # plotting second trajectory
    for k in range(x_tab2.shape[0]):
	    plt.plot(x_tab2[k,0],x_tab2[k,1],'+b',markersize=10)

    for k in range(x_tab2.shape[0]-1):
	    plt.plot([x_tab2[k,0], x_tab2[k+1,0]],[x_tab2[k,1], x_tab2[k+1,1]],'-b')
